...
public class operations {
	...
	static String displayall() {
		try {
			...
			String string_data = "";
			while (result.next()) {
				string_data = string_data + "   \t" + result.getInt(1) + "\t" + result.getString(2) + "\t"
	...
	static String display_one_by_one(int val, int p) {
		try {
			...
			/*
			 * String selectQuery="select * from product_table"; PreparedStatement
			 * preparedStatement;
			 * 
			 * preparedStatement=conn.prepareStatement(selectQuery); ResultSet
			...
			if (val == 2) {
				/*
				 * result.previous();
				 * string_data="   \t"+result.getInt(1)+"\t"+result.getString(2)+"\t"+result.
				 * getInt(3)+"\t"+result.getInt(4)+"\t"+result.getString(5)+"\n"; return
				 * string_data;
				 */
				if (result.isFirst()) {
